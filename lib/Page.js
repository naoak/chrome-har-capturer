...
Page.prototype.isDone = function () {
    ...
    return this.domLoadedTime && this.loadTime &&
        this.originalRequestId &&
        typeof this.originalRequestStatus != 'undefined' &&
        this.activeRequestIds.length == 0;
};
...
Page.prototype.getHAR = function (options) {
    var har = {
        'info': {
    ...
    if (options && options.pageComment) {
        har.info['comment'] = options.pageComment;
    }
...
function addActiveRequestId(page, id) {
    if (page.activeRequestIds.indexOf(id) == -1) {
        page.activeRequestIds.push(id);
    }
}

function removeActiveRequestId(page, id) {
    page.activeRequestIds.some(function(v, i) {
        if (v == id) {
            page.activeRequestIds.splice(i, 1);
        }
    });
}
